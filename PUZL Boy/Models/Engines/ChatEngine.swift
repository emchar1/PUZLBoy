    ...
    func playDialogue(level: Int, completion: ((Cutscene?) -> Void)?) {
        ...
        case 251:
            sendChatArray(items: [
                ...
                ChatItem(profile: .hero, imgPos: .left, chat: "\"Sweet summer child??\"")
            ]) { [unowned self] in
                handleDialogueCompletion(level: level, completion: completion)
            }
        case 276:
...
//            }
        ...
        case 301:
            if !dialogueWithCutscene[level]! { //PART 1
                let cutscene = CutsceneOldFriends()
                ...
                sendChatArray(items: [
                    ...
                    ChatItem(profile: .hero, imgPos: .left, chat: ".....wait, what do you mean we're leaving?! Where are we going?? 🥺"),
